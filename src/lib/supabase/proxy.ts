...
export async function refreshSession(request:NextRequest){
  ...
  if(error&&isAuthenticationServiceUnavailable(error)){
    void reportProductionError({source:"provider",message:"Supabase Auth is unavailable at the request boundary",severity:"error",fingerprint:"provider-degradation-supabase-auth"});
    const headers={"retry-after":"30","cache-control":"no-store"};
    return request.nextUrl.pathname.startsWith("/api/")
      ? NextResponse.json({error:{code:"AUTH_SERVICE_UNAVAILABLE",message:"Sign-in verification is temporarily unavailable. Please retry shortly."}},{status:503,headers})
      : new NextResponse("Sign-in verification is temporarily unavailable. Please retry shortly.",{status:503,headers});
  }

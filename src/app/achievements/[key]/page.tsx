...
export default async function AchievementDeepLink({params}:PageProps<"/achievements/[key]">){
  ...
  if(definitionResult.error||unlockResult.error)throw new Error("Achievement could not be loaded",{cause:definitionResult.error||unlockResult.error});
  const definition=definitionResult.data,unlock=unlockResult.data;

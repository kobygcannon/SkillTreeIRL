...
export async function PATCH(request: Request, context: RouteContext<"/api/v1/habits/[id]">) {
  try {
    ...
    if (!name || name.length > 180 || !days.length || days.some(day => !Number.isInteger(day) || day < 1 || day > 7) || !Number.isFinite(xp) || xp < 0 || xp > 500 || (minimum !== null && (!Number.isFinite(minimum) || minimum <= 0)) || (minimum !== null && !body.minimumUnit?.trim())) {

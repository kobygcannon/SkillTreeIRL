...
export async function POST(request:Request) {
  try {
    ...
    if(!name||name.length>180||xp<0||xp>500||!days.length||days.some(day=>!Number.isInteger(day)||day<1||day>7)||!isValidTimeZone(body.timezone||"UTC")||(minimum!==null&&(!Number.isFinite(minimum)||minimum<=0||!body.minimumUnit?.trim())))return NextResponse.json({error:{code:"VALIDATION_FAILED",message:"Check the habit name, preferred days, timezone, minimum target, and XP."}},{status:422});

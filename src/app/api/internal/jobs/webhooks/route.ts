...
async function run(request: Request) {
  ...
  if (!expected || provided !== `Bearer ${expected}`)
    return NextResponse.json(
      { error: { code: "UNAUTHORIZED", message: "Invalid worker credential" } },
      { status: 401 },
    );
  ...
  const { data: deliveries, error } = await admin
    ...
    .select("id,payload,attempts,webhooks(url,secret_ciphertext)")
  ...
  for (const delivery of deliveries || []) {
    ...
    if (leaseError) {
      await reportProductionError({
        ...
        context: { deliveryId: delivery.id, stage: "claim" },
      });
      failed++;
      continue;
    }
    ...
    const body = JSON.stringify(delivery.payload),
      ...
      signature = createHmac("sha256", decryptSecret(hook.secret_ciphertext))
        .update(`${timestamp}.${body}`)
        .digest("hex");
    try {
      const target = await assertPublicHttpsUrl(hook.url);
      const response = await fetch(target, {
        method: "POST",
        redirect: "error",
        headers: {
          "content-type": "application/json",
          "user-agent": "SkillTree-Webhooks/1.0",
          "x-skilltree-id": delivery.id,
          "x-skilltree-timestamp": String(timestamp),
          "x-skilltree-signature": `v1=${signature}`,
        },
        body,
      ...
      await reportProductionError({
        ...
        context: {
          ...
          attempts: attempt,
          dead,
          stateUpdateFailed: retryError?.message || null,
        },
      });
      failed++;
    }

...
function assertDb(result: DbResult, operation: string) {
...
function periodEnd(subscription: Stripe.Subscription) {
  ...
  return ends.length ? Math.max(...ends) : Math.floor(Date.now() / 1000);
}
async function reconcileSubscription(
  admin: NonNullable<ReturnType<typeof createAdminClient>>,
  subscription: Stripe.Subscription,
) {
  let organizationId = subscription.metadata.skilltree_organization_id;
  if (!organizationId) {
  ...
  if (organizationId) {
    const status = subscriptionStatus(subscription.status),
      expiresAt = new Date(periodEnd(subscription) * 1000).toISOString(),
      seats = subscription.items.data.reduce(
    ...
    assertDb(
      await admin
        ...
        .update({
          ...
          plan: "team",
          status,
          seat_quantity: Math.max(1, seats),
  ...
  let resolvedUser = subscription.metadata.skilltree_user_id;
  if (!resolvedUser) {
    const { data, error } = await admin
      .from("subscriptions")
      .select("user_id")
...
export async function POST(request: Request) {
  ...
  if (claimError) {
    ...
    if (readError)
      return NextResponse.json(
        { error: "Could not read event state" },
        { status: 500 },
      );
  ...
  try {
    if (event.type === "checkout.session.completed") {
      const session = event.data.object as Stripe.Checkout.Session,
        userId = session.metadata?.skilltree_user_id,
        organizationId = session.metadata?.skilltree_organization_id;
    ...
    if (
      ...
      const invoice = event.data.object as Stripe.Invoice,
        linked = invoice.parent?.subscription_details?.subscription;

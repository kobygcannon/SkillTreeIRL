...
async function reconcileSubscription(admin:NonNullable<ReturnType<typeof createAdminClient>>,subscription:Stripe.Subscription){
...
export async function POST(request:Request){
 ...
 try{
  ...
  if(event.type.startsWith("customer.subscription.")){
   await reconcileSubscription(admin,event.data.object as Stripe.Subscription);
  }
  if(event.type==="invoice.paid"||event.type==="invoice.payment_failed"){
   const invoice=event.data.object as Stripe.Invoice,linked=invoice.parent?.subscription_details?.subscription;
   const subscriptionId=typeof linked==="string"?linked:linked?.id;
   if(subscriptionId)await reconcileSubscription(admin,await stripe.subscriptions.retrieve(subscriptionId));
  }

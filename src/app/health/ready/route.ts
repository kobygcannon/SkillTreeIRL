...
export async function GET(){
  ...
  if(!configuration)return NextResponse.json({status:"not_ready",release:deploymentRelease(),checks:{configuration:false,legal,monitoring,database:false,requestProtection:false}},{status:503,headers:{"cache-control":"no-store"}});
  const admin=createAdminClient();if(!admin)return NextResponse.json({status:"not_ready",release:deploymentRelease(),checks:{configuration:true,legal,monitoring,database:false,requestProtection:false}},{status:503,headers:{"cache-control":"no-store"}});
  ...
  return NextResponse.json({status:ready?"ready":"not_ready",release:deploymentRelease(),checks:{configuration:true,legal,monitoring,database:!database.error,requestProtection:!requestProtection.error}},{status:ready?200:503,headers:{"cache-control":"no-store"}});
}

...
- Authorization and RLS have previously passed hostile tests in `supabase/tests/authorization_rls.test.sql`. The company-workspace migrations are installed in both provider environments and passed their 32-assertion rollback-only verification; the complete clean-database suite still requires a runnable CI or local Docker database.
- Goal measurement types, revisions, state changes, progress, reversals, skills, XP, quests, habits, achievements, evidence, entitlements, Stripe idempotency, exports, deletion, privacy defaults, notification preferences, offline idempotency, and large-account behavior are covered by the database suite and authenticated browser journey. Additive goal progress now uses an owner-authorised atomic function, supports measurement-specific entry, updates milestone thresholds, and returns an existing immutable event on a repeated idempotency key.
- Responsive/public/accessibility behavior: 16 public Playwright checks pass locally. They cover marketing/signup/help interactions, expandable content, the read-only conversion flow, ten public accessibility routes, all seven primary demo workspaces, and horizontal-overflow checks across the phone layout.
...
- Application quality in the current release candidate: 101 unit/static tests across 20 files, ESLint, strict TypeScript, and the optimized 99-route Next.js build pass. The expanded 16-check public Playwright suite includes automated accessibility inspection of every primary signed-out demo workspace and a phone-width overflow gate.
...
Goal-progress migrations `20260820160000_goal_progress_delta.sql` and `20260820161000_goal_progress_delta_idempotency.sql` were subsequently applied to both environments. Catalog verification confirmed the retry-safe function order, authenticated execution, and denied anonymous execution. The function deliberately uses a fixed empty search path and re-authorises `auth.uid()` against the goal owner before any write.
...
On 20 August 2026, Vercel production release `2f9a6fdeeb1dfb26840da031602423fb5ac9aa4e` completed successfully and `/health/live` returned HTTP 200 with that exact release. The complete 16-check public Playwright suite then passed against the live alias, including phone-width overflow, all primary demo workspaces, interactive accordions, signup handoff, and serious/critical accessibility checks. `/health/ready` returned HTTP 503 with database, request protection, Sentry monitoring, Resend email delivery, and VAPID browser push healthy; configuration/legal and billing were false. The probe therefore remains intentionally unavailable because legal/controller configuration and verified live Stripe billing are incomplete. The matching staging branch was promoted, but its stable alias remains protected by Vercel SSO; exact-release HTTP smoke evidence still requires the automation bypass. Always use the current `/health/live` response as the authoritative release identifier rather than this narrative record.
